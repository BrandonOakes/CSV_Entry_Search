...
class NewEntry():
	...
	def task_time(self):
       ...
       """
		value = True
		while value:
			self.time = input('How much time in minutes, was spent on your task?\n>')
			clear()
			try: 
				int(self.time)
...
with digits only. (ex. correct response -> 45 | incorrect response ->forty-five)\n\n\n""".format(self.time))
	...
	def task_notes(self):
		self.notes = input('''Do you have any additional notes to add about your task? (if not type -> none):\n>''').title()
		clear()
